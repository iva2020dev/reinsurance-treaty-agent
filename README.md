...
task in that set. The diagram below shows a real example selection —
both domain tasks actually implemented today, `burn_cost_check` and
`exclusion_completeness_checklist` — running in parallel after the
Verifier Node, not a mocked/hypothetical selection:
...
user selects in the running app — it always renders the same fixed
example selection, `MULTI_TASK_SELECTED_IDS` in `scripts/regenerate_
workflow_graph.py`, so it only changes if that example selection
itself is edited, or if the shared pipeline nodes/edges change).
